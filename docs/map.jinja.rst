...
We will see a quick setup to configure the ``searxng`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``searxng`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``searxng`` formulas under ``/srv/salt/searxng/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/searxng/parameters/dns:domain/
    ln -s dns:domain /srv/salt/searxng/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/searxng/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/searxng/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``searxng`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``searxng`` formulas under ``/srv/salt/searxng/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/searxng/parameters/roles
...
We create a configuration for the role ``searxng/server`` in ``/srv/salt/searxng/parameters/roles/searxng/server.yaml``:
...
We create another configuration for the role ``searxng/client`` in ``/srv/salt/searxng/parameters/roles/searxng/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles searxng/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles searxng/client
...
.. code-block:: console
    ...
    mv /srv/salt/searxng/parameters/dns:domain/  '/srv/salt/searxng/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-searxng-as-json:
      test.nop:
        - name: {{ searxng | json }}
...
Here is an example based on `template-formula/searxng/config/file.sls`_:
    ...
    searxng-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='searxng-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ searxng.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``searxng`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ searxng['winner'] }}
...
.. _template-formula/searxng/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/searxng/config/file.sls
